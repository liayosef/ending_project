...
class ParentHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed_path.path == '/register':
            ...
            # בניית HTML לרשומות - כל רשומה בנפרד
            history_entries = []
            for entry in filtered_history:
                domain = entry.get('domain', 'לא ידוע')
                ...
                history_entries.append(f'''
                    <div class="history-item">
                        <div class="domain-info">
                            ...
                            <div class="domain-time">{formatted_time} • {child_name}</div>
                        </div>
                        <div class="status-badge {status_class}">{status_text}</div>
                    </div>
                ''')

            # בניית כרטיסי סטטיסטיקה
            unique_domains = len(set(entry.get('domain', '') for entry in filtered_history))
            stats_cards = f'''
                <div class="stat-card">
                    <div class="stat-number">{len(filtered_history)}</div>
                    <div class="stat-label">רשומות מוצגות</div>
                </div>
                <div class="stat-card">
                    <div class="stat-number">{unique_domains}</div>
                    <div class="stat-label">דומיינים ייחודיים</div>
                </div>
                <div class="stat-card">
                    <div class="stat-number">{stats['blocked']}</div>
                    <div class="stat-label">ביקורים חסומים</div>
                </div>
                <div class="stat-card">
                    <div class="stat-number">{stats['allowed']}</div>
                    <div class="stat-label">ביקורים מותרים</div>
                </div>
            '''
            ...
            history_html = history_html.replace('${total_entries}', str(len(filtered_history)))
